    ...
    <template v-if="taskList.length">
      ...
      >
        ...
        <div class="listItem relative" :class="{'whiteBg': route.params.type == 2, 'mb-14': route.params.type != 3, 'mb-30': route.params.type == 3}" v-for="(item, index) in taskList" :key="item.id" @click="toRouter(item.id)">
          <!-- 执行人 -->
          <div v-if="route.params.type == 3" class="examinerWrap flex flex-between">
            <span class="flex1 flex" style="overflow: hidden">
              任务接收人：
              <span class="flex1 executorWrap">
...
const itemDeploy = {
  ...
  /* 内容label\key */
  content: [
      {label: "抽检类型", key: "scType", unitKey: "scResult"},
      {label: "工单编号", key: "workOrderNo"},
      {label: "料号", key: "fnumber"},
      {label: "抽检批号", key: "scBatchNo"},
      {label: "批号数量", key: "batchQty"},
      {label: "抽检时间", key: "scTime"},
  ],
  /* 根据状态值，对应下标 */
  status:  [
