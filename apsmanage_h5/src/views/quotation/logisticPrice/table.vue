<template>
  <div class="project-table">
    <LayoutModule title="物流费" class="layout-one">
    ...
    <!-- 规则弹窗-->
    <PxtPrompt @confirm="confirmButton" title="物流费" @cancel="resetFields" :show-prompt="rulePrompt.status">
      <el-form class="rule-form custom-input" ref="ruleFormRef" :model="ruleForm" :rules="ruleFormRules" :label-width="100" :hideRequiredAsterisk="true" label-position="left"
        ...
        <el-form-item label="物流项目" prop="projectId">
          <el-select v-model="ruleForm.projectId" value-key="paramValue" placeholder="选择物流项目">
            <el-option :label="item.paramName" :value="item.paramValue" v-for="item in logistics_project" :key="item.value"/>
          </el-select>
        </el-form-item>
        <el-form-item label="价格" prop="logisticsCost">
          <el-input v-model="ruleForm.logisticsCost" placeholder="4位小数，必须填写" :formatter="(value)=>formatMoney(value)" :parser="(value)=>formatMoneyParser(value)" inputmode="numeric"/>
        </el-form-item>
...
const {unit2, unit, logistics_project} = storeToRefs(useDictStore())
...
let data = reactive({
  ...
  ruleForm: {
    projectId: '',//物流项目
    unitId: '',  //单位
    logisticsCost: '', //物流报价费 4位小数
  },
  ruleFormRules: {
    projectId: [requiredValidate('请选择物流项目'), {
      validator: (rule, value, callback) => {
        projectIdValidate(rule, value, callback, props.tableDetail, 'projectId')
      }, trigger: 'change'
    }],
    unitId: [requiredValidate('请选择单位类型')],
    logisticsCost: [requiredValidate('请输入数量'), {validator: fourNumberValidate, trigger: 'blur'}],
  }
})
...
const confirmButton = () => {
  ruleFormRef.value.validate((valid, fields) => {
    if (valid) {
      emit('handelTable', undefined, {...ruleForm.value,logisticsCost:formatMoneyParser(ruleForm.value.logisticsCost)})
      resetFields()
    }
  })
}
