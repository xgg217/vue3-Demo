<template>
  <div class="demandFormWrap">
    <el-form ref="ruleFormRef" :model="demandForm" label-width="24vw" label-position="left" :disabled="!isEdit" class="demandForm" :class="{'busCancelEdit': !isEdit}" :rules="formRule">
      ...
      <el-form-item label="客户名称" prop="customerName" :rules="[{ validator: checkCustomName, trigger: 'change' }]" id="customerName" v-if="!isQuota">
        <remoteSelectInput ref="remoteSelectInputRef" v-if="isEdit && demandForm.status == null && route.name == 'addDemand'" @getCustomerInfo="getCustomerInfo" :initInfo="{id: demandForm.customerId, name: demandForm.customerName}"></remoteSelectInput>
        <el-input v-else :disabled="demandForm.status > -1 " v-model="demandForm.customerName"></el-input>
      </el-form-item>

<!--      <el-form-item label="客户" v-if="isQuota && route.params.id">-->
<!--        {{ demandForm.customerName }}-->
<!--      </el-form-item>-->
      ...
      <template v-if="isQuota && !isEdit">
        ...
        <el-form-item label="合作方式" label-width="24vw">
        ...
        <el-form-item label="加工工序" v-if="!isQuotaInitiateQuotation">
          <div>
            ...
            <van-icon v-if="demandForm.customerBaseId" class="showMoreIcon" :name="!quoteShowMore ? 'arrow-down' : 'arrow-up'" @click="changeQuoteShowMore"/>
      ...
      <el-form-item label="签约工厂" prop="contractFactoryId" id="contractFactoryId" v-if="!isQuota">
        ...
        <span v-else class="relative">{{ (demandForm.contractFactoryId && demandForm.contractFactoryId != 0) ? factory[demandForm.contractFactoryId * 1 - 1].label : '--' }}</span>
      </el-form-item>

      <el-form-item :label="(isQuota && !isEdit) ? '' : '合作方式'" label-width="24vw">
        ...
        <div class="innerFormItem" :class="{'mrgin5': !isEdit}" v-if="(demandForm.cooperateType || demandForm.cooperateType === 0) && quoteShowMore">
          ...
          <el-form-item label="入料途径" label-width="24vw" v-if="demandForm.cooperateType === 1" id="material" :class="{'changeMarginTo5': !isEdit}">
            <div>
          ...
          <el-form-item label="" label-width="24vw" v-if="demandForm.cooperateType === 1" id="packageMaterial" :class="{'changeMarginTo5': !isEdit}">
            <div>
          ...
          <template v-if="demandForm.cooperateType != 3">
            <el-form-item label="物料损耗率" label-width="24vw" :rules="[
            ...
            <el-form-item label="" label-width="24vw" :rules="[
          ...
          <el-form-item label="成品报废率" label-width="24vw" :rules="[
              { validator: checkAdvanceRate, trigger: 'change' },
          ...
          <el-form-item label="结工单要求" label-width="24vw" v-if="demandForm.cooperateType != 3 && !isQuota" prop="workOrderAsk" id="workOrderAsk">
          ...
          <el-form-item label="出货要求" label-width="24vw" v-if="demandForm.cooperateType != 3" prop="shipmentAskList" id="shipmentAskList">
          ...
          <el-form-item label="回购条款" label-width="24vw" v-if="demandForm.cooperateType != 3 && !isQuota" prop="buybackProvision" id="buybackProvision">
            <div style="width: 100%">
          ...
          <el-form-item label="付款条件" label-width="24vw" :class="{'changeMaterialCheckMargin': isEdit, 'changeMarginTo5': !isEdit}" v-if="!isQuota">
          ...
          <el-form-item label="开票要求" label-width="24vw" :prop="demandForm.cooperateType == 3 ? 'invoiceAskListRadio' : 'invoiceAskList'" :id="demandForm.cooperateType == 3 ? 'invoiceAskListRadio' : 'invoiceAskList'" v-if="!isQuota">
            <template v-if="demandForm.cooperateType != 3">
          ...
          <el-form-item label="售后条件" label-width="24vw" v-if="demandForm.cooperateType == 3">
          ...
          <el-form-item label="对等性" label-width="24vw" v-if="demandForm.cooperateType == 3">
...
<script setup>
  import remoteSelectInput from "@/components/business/remoteSelectInput.vue"
  import remoteSelectPro from "@/components/business/remoteSelectPro.vue"
...
import delete$ from "dingtalk-jsapi/api/biz/cspace/delete"
  ...
  const props = defineProps(["submitForm", "setMeetingInfo", "outId"])
  ...
  if (route.fullPath.indexOf("/quote/") > -1 || route.name === "processDetail") {
    isQuota.value = true
  }
  ...
  const getFormData = (outId) => {
    openLoading()
    // let api=outId?getHttpInternalOfferBusinessInfo:customerDetailInfo
    customerDetailInfo(route.params.id * 1 || outId)
      .then((res) => {
        if (res.code == 200) {
          if (res.msg == '暂无承载数据') {
            closeLoading()
  ...
  const setViewDetail=(info)=>{
      const {status=13, id, customerName='' } = info
      businessStore.updatePageInfo({state: status, cusId: id, cusName: customerName})
      businessStore.updateData(info)
  ...
  watch(() => props.outId, (val)=> {
    if(isQuota.value && val){
      getFormData(val)
    }
  }, { immediate: true })
  ...
  const submit = async (flag) => {
    ...
    if (!flag) {
      props.submitForm(demandForm, flag)
      return
    }
  ...
  onMounted(() => {
    ...
    if (route.params.id && route.name != "processDetail") {
      getFormData()
    }
  })
  ...
  const setViewData = (info) => {
    ...
    demandForm.value = info
    if (demandForm.value.status > 0) {
      if (demandForm.value.status === 6 && route.name === 'customerDemand') {
        isEdit.value = true
        return
      }
      isEdit.value = false
