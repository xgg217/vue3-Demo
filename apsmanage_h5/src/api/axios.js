...
serve.interceptors.request.use(
    (config) => {
        ...
        /* 关联taskId */
        if (taskId) {
            config.headers.flowable_process_instance_ids = taskId
            config.headers.flowable_process_instance_id = encodeURIComponent(JSON.stringify({processInstanceId: taskId}))
        }
        return config
    },
    (error) => {
...
serve.interceptors.response.use(
    res => {
        ...
        if (res.data.code === 200) {
            ...
            !res.config?.notify && Notify(res.data.msg);
